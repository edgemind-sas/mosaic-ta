...
fig_mod1_perf = px.line(mod1_perf, title=f"Modèle 1 : Performance de la stratégie",
                  markers=True,
                  labels=dict(
                      value="Performance"))
...
Afin d'éliminer les variations liées aux différentes actions d'achats/ventes, nous pouvons lisser la
performance en utilisant par exemple un minimum glissant sur une journée, c.-à-d. de taille 12*24 = 288 unité de
temps.
#+BEGIN_SRC python :session start :results silent :exports code :tangle start.py
mod1_smooth_perf = mod1_perf.rolling(12*24).mean()
fig_mod1_smooth_perf = px.line(mod1_smooth_perf, title=f"Modèle 1 : Performance lissée (min) de la stratégie",
                               markers=True,
                               labels=dict(
                                   value="Performance"))
fig_mod1_smooth_perf.update_layout(showlegend=False)
fig_mod1_smooth_perf
#+END_SRC
#+BEGIN_SRC python :session start :results html :exports results
pio.to_html(fig_mod1_smooth_perf, include_plotlyjs="cdn",
            full_html=False,
            config={'displayModeBar': False})
#+END_SRC
...
mod2_trades_buy = pd.Series(0, index=target_test.index, name="achat")
mod2_trades_sell = (target_test.copy().rename("vente") + 1)*invest

mod2_trades_buy[idx_mod2_pred_buy] = -invest
mod2_trades_sell[~idx_mod2_pred_buy] = 0
mod2_trades_sell = mod2_trades_sell.shift(k_horizon).fillna(0)

mod2_trades_balance = (mod2_trades_buy + mod2_trades_sell).rename("portefeuille")
mod2_trades_balance.iloc[0] += 1


mod2_trades_df = pd.concat([mod2_trades_buy,
                            mod2_trades_sell,
                            mod2_trades_balance], axis=1)
#+END_SRC
...
idx_mod2_pred_buy = mod2_target_pred > buy_thresh
idx_mod2_pred_sell = idx_mod2_pred_buy.shift(k_horizon)

mod2_trades_usdt = pd.Series(0, index=target_test.index, name="USDT")

mod2_trades_order_usdt = pd.Series(0, index=target_test.index, name="Order USDT")
mod2_trades_order_btc = pd.Series(0, index=target_test.index, name="Order BTC")

mod2_trades_order_usdt[idx_mod2_pred_buy] = -1
mod2_trades_order_btc[idx_mod2_pred_buy] = ohlcv_test_df.loc[mod2_target_pred.index[idx_mod2_pred_buy]]**(-1)
...
Visualisation de la performance de la stratégie sur la période de test.
#+BEGIN_SRC python :session start :results silent :exports code :tangle start.py
mod2_perf = mod2_trades_balance.cumsum().rename("perf")
fig_mod2_perf = px.line(mod2_perf, title=f"Modèle 2 : Performance de la stratégie",
                  markers=True,
                  labels=dict(
                      value="Performance"))
...
Visualisation de la performance lissée.
#+BEGIN_SRC python :session start :results silent :exports code :tangle start.py
mod2_smooth_perf = mod2_perf.rolling(12*24).mean()
fig_mod2_smooth_perf = px.line(mod2_smooth_perf, title=f"Modèle 2 : Performance lissée (min) de la stratégie",
                               markers=True,
                               labels=dict(
                                   value="Performance"))
fig_mod2_smooth_perf.update_layout(showlegend=False)
fig_mod2_smooth_perf
#+END_SRC
#+BEGIN_SRC python :session start :results html :exports results
pio.to_html(fig_mod2_smooth_perf, include_plotlyjs="cdn",
            full_html=False,
            config={'displayModeBar': False})
#+END_SRC
...
période d'étude :
#+BEGIN_SRC python :session start :results silent :exports code :tangle start.py
ohlcv_test_df = ohlcv_df.iloc[nb_train:]
ohlcv_test_perf = (ohlcv_test_df["close"]/ohlcv_test_df["close"].iloc[0]).rename("perf")
#+END_SRC
...
models_perf_df = pd.concat([
    ...
    mod1_smooth_perf.reset_index().assign(model="modèle 1"),
    mod2_smooth_perf.reset_index().assign(model="modèle 2"),
    ], axis=0)
