...
baissiers, cf. Figure [[fig-candles]].
...
Nous pouvons visualiser les nuages de points de la variable cible avec les différentes
variables explicatives en y ajoutant une droite de régression linéaires (option =trendline= de la
fonction =px.scatter=).
#+BEGIN_SRC python :session start :results silent :exports code :tangle start.py
data_indic = pd.concat([indics_df, returns], axis=1)
data_target_features_long_df = \
    pd.melt(data_indic.reset_index().head(10000),
...
fig_target_pairs = \
    px.scatter(
        ...
        y=returns.name,
        trendline="ols",
        title=f"Variable cible {returns.name} en fonction de l'indicateur RSI_5",
    )
fig_target_pairs
#+END_SRC
#+BEGIN_SRC python :session start :results html :exports results
...
En ne faisant ressortir uniquement que le résultat de la régression, on peut observer une corrélation entre la valeur de l'indicateur et le rendement.
(Attention bien faire attention à l'échelle)

#+BEGIN_SRC python :session start :results silent :exports code :tangle start.py
fig_target_pairs.update_traces(visible=False, selector=dict(mode="markers"))
fig_target_pairs
#+END_SRC
#+BEGIN_SRC python :session start :results html :exports results
pio.to_html(fig_target_pairs, include_plotlyjs="cdn",
...
# Signaux de vente
idx_mod1_pred_sell = idx_mod1_pred_buy.shift(k_horizon).fillna(False)
#+END_SRC
...
Sur la figure [[fig-strategie]] un exemple de la stratégie mentionnée ci-dessus. On investit à chaque fois que le rendement prédit est positif.
Dans notre stratégie, le montant d'USDT investi est identique (pour les rendements positifs) quelque soit la période, mais on pourrait imaginer une stratégie avec des investissements variables.
Le nombre de BTC acquis est calculé avec le prix du BTC à l'ouverture de la période.

#+NAME: fig-strategie
#+CAPTION: Stratégie d'investissement.
[[./strategy_invest.png]]

On revend tous les bitcoins obtenus à la période p à la période p+k_horizon.
On prend comme prix de conversion le prix du BTC à la fermeture de la période.
...
fig_mod1_perf.update_yaxes(range=[0, mod1_trades_df["USDT:perf"].max()*1.05])
