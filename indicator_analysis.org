...
#+NAME: eq:score_awer
\begin{equation}
\text{AWER}(\alpha, k| i_{m}) = \left(1 - \alpha - \frac{\pi^{\pm}_{i_{m}}}{2}\right)\rho^{\text{h}}_{p + k,\alpha|i_{m}}  + \left(\alpha - \frac{\pi^{\pm}_{i_{m}}}{2}\right)\rho^{\text{l}}_{p + k,1 - \beta|i_{m}} + \pi^{\pm}_{i_{m}} \overline{\rho}^{\text{c}}_{p+k | i_{m}}
\end{equation}
...
#+NAME: eq:score_awer
\begin{equation}
\text{PAWER}(\alpha, k | i_{m}) =  \text{AWER}(\alpha, k | i_{m}) - \text{AWER}_{\text{ref}}(\alpha, k | \varnothing),
\end{equation}
